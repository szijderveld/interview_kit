...
async def test_save_and_load_conversation_round_trip() -> None:
...
async def test_load_unknown_conversation_raises() -> None:
    store = InMemoryConversationStore()
...
async def test_save_and_load_session_round_trip() -> None:
    store = InMemoryConversationStore()
    conv = _conv()
    await store.save_conversation(conv)
    s = _session(conv)
...
async def test_load_unknown_session_raises() -> None:
    store = InMemoryConversationStore()
    with pytest.raises(KeyError):
        await store.load_session("missing")


async def test_update_session_state_mutates_via_copy() -> None:
    store = InMemoryConversationStore()
    conv = _conv()
    await store.save_conversation(conv)
    s = _session(conv)
    ...
    # The original frozen instance is unchanged (Pydantic returned a copy).
    assert s.state is SessionState.CREATED


async def test_update_session_state_unknown_raises() -> None:
    store = InMemoryConversationStore()
    with pytest.raises(KeyError):
...
async def test_append_and_list_turns_preserves_order() -> None:
    store = InMemoryConversationStore()
    conv = _conv()
    await store.save_conversation(conv)
...
async def test_list_turns_for_session_with_no_turns_is_empty() -> None:
    store = InMemoryConversationStore()
    conv = _conv()
    await store.save_conversation(conv)
    await store.save_session(_session(conv))
...
async def test_list_turns_returns_copy_not_internal_list() -> None:
    store = InMemoryConversationStore()
    conv = _conv()
    await store.save_conversation(conv)
    await store.save_session(_session(conv))
    ...
    snapshot.clear()
    # Internal list should not be affected by external mutation.
    assert len(await store.list_turns("s1")) == 1


async def test_append_turn_for_unknown_session_raises() -> None:
    store = InMemoryConversationStore()
    with pytest.raises(KeyError):
        await store.append_turn(
            "missing", Turn(index=0, speaker="agent", text="x", timestamp=_ts())
        )
...
async def test_save_and_load_runtime_state() -> None:
    store = InMemoryConversationStore()
...
async def test_load_runtime_state_missing_returns_none() -> None:
    store = InMemoryConversationStore()
    assert await store.load_runtime_state("missing") is None


async def test_save_runtime_state_overwrites_previous() -> None:
    store = InMemoryConversationStore()
...
async def test_save_and_load_extract() -> None:
    store = InMemoryConversationStore()
    ex = Extract(
        session_id="s1",
...
async def test_load_extract_missing_returns_none() -> None:
    store = InMemoryConversationStore()
