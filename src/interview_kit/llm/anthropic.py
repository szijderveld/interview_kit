...
Two sequential Anthropic calls per agent turn:
...
2. :meth:`compose_utterance` — Claude Sonnet 4.6 with plain-text
   streaming. Yields tokens as they arrive so the voice transport can
   begin TTS playback before the full utterance is generated.
...
``session_id`` and ``completed_at``, and the wrapper reconstructs
``full_transcript`` from the input, so the LLM is only asked to fill in
``goal_statuses`` + ``unprompted_findings``.
...
/ ``last_extract_usage`` as a side channel — the runner reads these
after every call and threads the numbers into ``turn_recorded`` events
and the ``completed`` event's ``eval_usage_totals``. The
:class:`LLMClient` protocol does not declare these attributes; only
this concrete implementation surfaces them.

Retry / backoff lives in the loop, not here. This client raises on the
SDK's failure types (``anthropic.APIError``, ``pydantic.ValidationError``)
and lets the runner's retry policy handle them.
"""
...
class _ExtractToolInput(BaseModel):
    ...
    ``session_id`` and ``completed_at``, and asking the LLM to echo back
    ``full_transcript`` (which we just sent it) wastes tokens. The
    wrapper reconstructs the full Extract from this plus the inputs.
    """
...
class AnthropicLLMClient:
    ...
    async def derive_extract(
        ...
        # session_id and completed_at are runner-owned placeholders;
        # the runner overwrites them after this returns.
        return Extract(
            session_id=conv.id,
...
def _cached_system_blocks(text: str) -> list[dict[str, Any]]:
    """One ephemeral-cached text block (5-minute prompt cache)."""
    return [
        {
            "type": "text",
            "text": text,
