...
- ``goal_status_changed`` is emitted **only at completion**. The loop's
  mid-session goal-status updates are an internal hint table; the
...
- ``turn_recorded.payload`` includes LLM usage telemetry for the agent's
  compose call: ``tokens_in``, ``tokens_out``, ``cache_read_tokens``,
