"""``interview_kit`` console script — thin shim over the public API.

Subcommands:

- ``interview_kit demo`` — run the shipped simulator example. No API key
  required by default; ``--use-anthropic`` swaps in the real LLM.
- ``interview_kit simulate <conversation.yaml>`` — load a Conversation
...
- ``interview_kit --version`` — print the installed package version.
...
from interview_kit import Conversation, Engine, EvalResult, Extract
from interview_kit.examples import simulated as simulated_demo
...
def _package_version() -> str:
    try:
        return metadata.version("interview_kit")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version", action="version", version=f"interview_kit {_package_version()}"
    )
