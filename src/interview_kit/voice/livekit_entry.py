...
5. apply the :class:`EvalResult` to the loop-time goal-status table;
...
7. accumulate ``compose_utterance`` into a single text, validate, regen
   once on phrasing failure, then push one or more :class:`ChatChunk`\\
   s into the LLMStream so AgentSession can stream to TTS;
8. flush :class:`SessionRuntimeState` and append the agent Turn.
...
Phrasing regen in voice mode diverges from the simulator runner: the
runner streams compose chunks, validates after the stream exhausts, and
...
Cancel and disconnect. ``cancel_session`` writes ABANDONED to the store
and deletes the LiveKit room; AgentSession exits on disconnect and
``entrypoint`` finalises depending on the observed terminal state. The
chat() body also short-circuits if it observes ABANDONED at the top of
an iteration.
"""
...
from interview_kit.protocols import ConversationStore, EventSink, LLMClient
...
def _read_usage(llm: LLMClient, attr: str) -> dict[str, int]:
    """Read ``llm.<attr>`` if surfaced; else zeroed Usage."""
...
def rehydrate_state(
    ...
    """Apply persisted runtime state + transcript hints to ``state``.

    Mirrors :func:`interview_kit.loop.runner._resume_bootstrap`: counters
...
class _InterviewerStream(LLMStream):
    ...
    async def _run(self) -> None:
        ...
        # Cancel-mid-flight observation. If the operator cancelled
        ...
        # in one chunk rather than streaming both attempts, to avoid
        # speaking the failed candidate out loud.
        transcript = await state.store.list_turns(state.session_id)
...
async def _compose_full_text(
    ...
    Voice-mode divergence: the simulator runner streams chunks to its
    caller and validates after the stream exhausts, accepting that a
...
def build_agent_session(state: PerSessionState) -> AgentSession:
    ...
    Plugin imports are lazy so ``import interview_kit.voice.livekit_entry``
...
async def _finalize_extract(engine: Engine, state: PerSessionState) -> None:
    """Run the canonical derive_extract pass and emit ``completed``."""
    transcript = await state.store.list_turns(state.session_id)
