...
When ``livekit`` is configured, ``provision_session`` mints a real
...
and ``entrypoint`` delegates to :mod:`interview_kit.voice.livekit_entry`.

State transitions emit ``SessionEvent``s through the configured sink.
``goal_status_changed`` events are only emitted at completion, inside
the canonical extract pass — the loop-time goal status table is a hint,
not canonical truth.
"""
...
class Engine:
    ...
    async def provision_session(
        ...
        """Snapshot the Conversation onto a new Session and issue creds.

        Snapshotting on the Session means mid-call edits to the underlying
        Conversation never affect an in-flight session. State → READY.
        Emits ``session_provisioned``.
        """
    ...
    async def cancel_session(self, session_id: str, reason: str = "") -> None:
        ...
        unwind via its room-disconnect handler.
        """
        session = await self.store.load_session(session_id)
        if session.state in _TERMINAL_STATES:
            raise ValueError(
        ...
        if self.livekit is not None:
            from interview_kit.voice.livekit_entry import delete_room
    ...
    async def entrypoint(self, ctx: object) -> None:
        ...
        from interview_kit.voice.livekit_entry import run_entrypoint
    ...
    def _mint_credentials(
        ...
        (24-hour TTL). Simulator-only deployments get stub credentials
        with a sentinel URL so consumer code can still flow.
        """
        room_name = f"iv:{session_id}"
        if self.livekit is not None:
            from interview_kit.voice.livekit_entry import mint_join_token
