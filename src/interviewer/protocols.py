...
All I/O-bearing methods are ``async def`` (D3). Engine code awaits them.
...
- ``LLMClient`` — the brain. Each agent turn is two sequential calls per
  D2: ``evaluate_turn`` (structured JSON, small/fast model) followed by
...
class LLMClient(Protocol):
    """The brain. Two sequential calls per agent turn (D2)."""
...
class RespondentSimulator(Protocol):
    """Synthetic respondent for ``simulate_session``. Text-mode only in v1."""
