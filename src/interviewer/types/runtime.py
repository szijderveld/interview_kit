...
Match SCOPE.md → Public API → Runtime types. Frozen Pydantic models;
mutate via ``model_copy(update={...})``. The Conversation a Session ran
against is snapshotted onto ``Session.conversation_snapshot`` so mid-call
edits to the underlying Conversation never affect an in-flight session
(D10).
"""
...
class Turn(BaseModel):
    ...
    text: str
    timestamp: datetime
    # Live hint set by evaluate_turn; NOT canonical (see SCOPE Canonical truth).
